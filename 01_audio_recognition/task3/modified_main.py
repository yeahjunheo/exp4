...
# State variables for playback and microphone input control
is_playing = False
...
def select_song():
    ...
    # Pause playback and microphone input when popup is active
    global is_playing, is_input_active
    is_playing = False
    is_input_active = False
    if 'stream_play' in globals():
        stream_play.stop_stream()

    select_song_popup.wait_window()  # Wait until popup is closed

    # Resume playback and microphone input after popup closes
    is_playing = True
...
text.set("0.0")
label = tkinter.Label(master=remote_frame, textvariable=text, font=("", 30))
label.pack()
